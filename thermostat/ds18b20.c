...
void ds_write1bit(void)
{
    ...
    // Pull the 1-Wire bus low to initiate the write time slot
    DDRB |= (1 << PB4);         // Set the port bit for output
    _delay_us(6);               // Hold the bus low for at least 6µs (as per DS18B20 datasheet)
    DDRB &= ~(1 << PB4);        // Release the bus within 15µs
    _delay_us(64);              // Complete the time slot (total duration at least 60µs)
...
void ds_write0bit(void)
{
    ...
    // Pull the 1-Wire bus low to initiate the write time slot
    DDRB |= (1 << PB4);         // Set the port bit for output
    _delay_us(60);              // Hold the bus low for at least 60µs
    DDRB &= ~(1 << PB4);        // Release the bus
    _delay_us(10);              // Allow for at least 1µs recovery time between individual write slots
...
unsigned char ds_readbit(void)
{
    ...
    // Pull the 1-Wire bus low to initiate the read time slot
    DDRB |= (1 << PB4);         // Set the port bit for output
    _delay_us(6);               // Hold the bus low for at least 6µs
    DDRB &= ~(1 << PB4);        // Release the bus within 9µs
    _delay_us(9);               // Allow a bit of time for the line to settle before reading

    // Read the value on the 1-Wire bus
    result = (PINB & (1 << PB4)) ? 1 : 0;

    _delay_us(55);              // Complete the time slot (total duration at least 60µs)
